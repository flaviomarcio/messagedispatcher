...
public:
    ...
    static const QVariant getAlpha(const QVariant&v){
        ...
        if(v.typeId()==QMetaType::LongLong || v.typeId()==QMetaType::ULongLong)
    ...
    QVariant parseInterval(const QVariant&v, const QVariant&defaultV=QVariant()){
        ...
        if(v.typeId()==QMetaType::LongLong || v.typeId()==QMetaType::ULongLong || v.typeId()==QMetaType::Int || v.typeId()==QMetaType::UInt || v.typeId()==QMetaType::Double)
            return v;
...
QDateTime DateUtil::firstMonthDate(const QVariant&v) const
{
    ...
    vv=vv.typeId()==QMetaType::QDate?vv.toDate():QDate::currentDate();
    auto d=(vv.isValid()?vv.toDateTime():vv.typeId()==QMetaType::QDate?QDateTime(vv.toDate(), static_minTime):QDateTime()).date();
...
QDateTime DateUtil::lastMonthDate(const QVariant&v) const
{
    auto vv=(v.isValid()?v:*this);
    vv=vv.typeId()==QMetaType::QDate?vv.toDate():QDate::currentDate();
    auto d=(vv.isValid()?vv.toDateTime():vv.typeId()==QMetaType::QDate?QDateTime(vv.toDate(), static_maxTime):QDateTime()).date();
...
const QDateTime DateUtil::toDateTime(const QVariant &v)
{
    ...
    if(this->typeId()==QMetaType::QDate){
        auto v=QDateTime(QVariant::toDate(), QTime());
        return v;
    }

    if(this->typeId()==QMetaType::QTime){
        auto v=QDateTime(QDate(), QVariant::toTime());
        return v;
    }

    if(this->typeId()==QMetaType::QString || this->typeId()==QMetaType::QByteArray){
...
const QDate DateUtil::toDate(const QVariant &v)
{
    ...
    if(this->typeId()==QMetaType::QDateTime)
...
bool DateUtil::checkBetween(const QVariant &v, QDateTime &vMin, QDateTime &vMax)
{
    ...
    else if(vv.typeId()==QMetaType::QString || vv.typeId()==QMetaType::QByteArray){
    ...
    else if(vv.typeId()==QMetaType::QDate || vv.typeId()==QMetaType::QDateTime){
